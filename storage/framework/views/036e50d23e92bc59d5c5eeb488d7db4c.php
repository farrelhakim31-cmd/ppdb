...
<div class="container-fluid p-4">
            ...
            <div class="card h-100">
                <div class="card-body p-4">
                    ...
                    <div class="list-item d-flex justify-content-between align-items-center">
                        <div><span class="fw-semibold"><?php echo e($j->nama); ?></span><br><span class="badge badge-custom <?php echo e($j->is_active ? 'bg-success' : 'bg-secondary'); ?>"><?php echo e($j->is_active ? 'Aktif' : 'Tidak Aktif'); ?></span></div>
                        <div>
                            <button class="btn btn-sm btn-outline-primary rounded-pill" onclick="editJurusan(<?php echo e($j->id); ?>,'<?php echo e($j->kode); ?>','<?php echo e($j->nama); ?>','<?php echo e($j->deskripsi); ?>',<?php echo e($j->kuota); ?>,<?php echo e($j->is_active ? 'true' : 'false'); ?>)"><i class="fas fa-edit"></i></button>
                        </div>
                    </div>
        ...
        <div class="col-md-4">
            <div class="card h-100">
                <div class="card-body p-4">
                    ...
                    <?php $__currentLoopData = $jurusan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $j): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="list-item d-flex justify-content-between align-items-center">
                        <span class="fw-semibold"><?php echo e($j->nama); ?></span><span class="badge badge-custom bg-info"><?php echo e($j->kuota); ?> siswa</span>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="col-md-4">
            <div class="card h-100">
                <div class="card-body p-4">
                    ...
                    <div class="list-item d-flex justify-content-between align-items-center">
                        <div><span class="fw-semibold"><?php echo e($g->nama); ?></span><br><span class="badge badge-custom <?php echo e($g->is_active ? 'bg-success' : 'bg-secondary'); ?>"><?php echo e($g->is_active ? 'Aktif' : 'Belum Aktif'); ?></span></div>
                        <div>
                            <button class="btn btn-sm btn-outline-primary rounded-pill" onclick="editGelombang(<?php echo e($g->id); ?>,'<?php echo e($g->nama); ?>','<?php echo e($g->tanggal_mulai); ?>','<?php echo e($g->tanggal_selesai); ?>',<?php echo e($g->biaya_pendaftaran); ?>,<?php echo e($g->is_active ? 'true' : 'false'); ?>)"><i class="fas fa-edit"></i></button>
                        </div>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="row mt-5">
        <div class="col-12">
            ...
            <div class="row g-4">
                <div class="col-md-3">
                    <form method="POST" action="<?php echo e(route('admin-panitia.sync-data')); ?>"><?php echo csrf_field(); ?>
                ...
                <div class="col-md-3">
                    <form method="POST" action="<?php echo e(route('admin-panitia.backup-data')); ?>"><?php echo csrf_field(); ?>
                ...
                <div class="col-md-3">
                    <form method="POST" action="<?php echo e(route('admin-panitia.clear-cache')); ?>"><?php echo csrf_field(); ?>
...
document.getElementById('jurusanModal').addEventListener('hidden.bs.modal',function(){document.getElementById('jurusanForm').action='<?php echo e(route("admin-panitia.store-jurusan")); ?>';document.getElementById('jurusan_method').value='';document.getElementById('jurusanForm').reset();document.getElementById('jurusan_active_check').style.display='none'})
document.getElementById('gelombangModal').addEventListener('hidden.bs.modal',function(){document.getElementById('gelombangForm').action='<?php echo e(route("admin-panitia.store-gelombang")); ?>';document.getElementById('gelombang_method').value='';document.getElementById('gelombangForm').reset();document.getElementById('gelombang_active_check').style.display='none'})
</script>
<div class="modal fade" id="biayaModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><form method="POST" action="<?php echo e(route('admin-panitia.store-pembayaran')); ?>"><?php echo csrf_field(); ?><div class="modal-header"><h5 class="modal-title">Atur Biaya</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="mb-3"><label class="form-label">Nama Biaya</label><input type="text" class="form-control" name="nama" required></div><div class="mb-3"><label class="form-label">Nominal</label><input type="number" class="form-control" name="nominal" min="0" required></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button><button type="submit" class="btn btn-primary">Simpan</button></div></form></div></div></div>

<div class="modal fade" id="dokumenModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><form method="POST" action="<?php echo e(route('admin-panitia.store-dokumen')); ?>"><?php echo csrf_field(); ?><div class="modal-header"><h5 class="modal-title">Kelola Syarat Berkas</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="mb-3"><label class="form-label">Nama Dokumen</label><input type="text" class="form-control" name="nama" required></div><div class="mb-3 form-check"><input type="checkbox" class="form-check-input" name="is_required" checked><label class="form-check-label">Wajib</label></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button><button type="submit" class="btn btn-danger">Simpan</button></div></form></div></div></div>

<div class="modal fade" id="wilayahModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><form method="POST" action="<?php echo e(route('admin-panitia.store-provinsi')); ?>"><?php echo csrf_field(); ?><div class="modal-header"><h5 class="modal-title">Kelola Wilayah</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="mb-3"><label class="form-label">Nama Wilayah</label><input type="text" class="form-control" name="nama" required></div><div class="mb-3"><label class="form-label">Kode Pos</label><input type="text" class="form-control" name="kode" required></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button><button type="submit" class="btn btn-primary">Simpan</button></div></form></div></div></div>

<div class="modal fade" id="infoPpdbModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content"><form method="POST" action="<?php echo e(route('admin-panitia.update-info-ppdb')); ?>"><?php echo csrf_field(); ?><div class="modal-header"><h5 class="modal-title">Informasi PPDB</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="row"><div class="col-md-6 mb-3"><label class="form-label">Nama Sekolah</label><input type="text" class="form-control" name="nama_sekolah" value="<?php echo e($infoPpdb['nama_sekolah']); ?>" required></div><div class="col-md-6 mb-3"><label class="form-label">Tahun Ajaran</label><input type="text" class="form-control" name="tahun_ajaran" value="<?php echo e($infoPpdb['tahun_ajaran']); ?>" required></div><div class="col-12 mb-3"><label class="form-label">Alamat Sekolah</label><textarea class="form-control" name="alamat_sekolah" rows="2" required><?php echo e($infoPpdb['alamat_sekolah']); ?></textarea></div><div class="col-md-6 mb-3"><label class="form-label">Telepon</label><input type="text" class="form-control" name="telepon_sekolah" value="<?php echo e($infoPpdb['telepon_sekolah']); ?>" required></div><div class="col-md-6 mb-3"><label class="form-label">Email</label><input type="email" class="form-control" name="email_sekolah" value="<?php echo e($infoPpdb['email_sekolah']); ?>" required></div><div class="col-12 mb-3"><label class="form-label">Kepala Sekolah</label><input type="text" class="form-control" name="kepala_sekolah" value="<?php echo e($infoPpdb['kepala_sekolah']); ?>" required></div></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button><button type="submit" class="btn btn-primary">Simpan</button></div></form></div></div></div>

</body>
</html><?php /**PATH C:\Users\PC-08\Farrel_ujikom\resources\views/admin-panitia/master-data.blade.php ENDPATH**/ ?>