...
@section('title', 'Info PPDB - SMK BAKTI NUSANTARA 666')
...
<div class="min-h-screen bg-gray-50">
    ...
    <header class="bg-white shadow-sm border-b">
        <div class="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <a href="{{ Auth::check() && Auth::user()->role === 'siswa' ? route('siswa.dashboard') : route('home') }}" class="text-gray-600 hover:text-blue-600 transition-colors">
                ← Kembali
            </a>
        </div>
    </header>
    ...
    <div class="max-w-4xl mx-auto px-4 py-12">
        ...
        <div class="grid md:grid-cols-2 gap-6 mb-12">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
                ...
                <form action="{{ route('ppdb.check-status') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-start">
                            ...
                            <div>
                                <p class="font-semibold text-gray-900">{{ $mapSettings->school_name }}</p>
                                <p class="text-gray-600">{{ $mapSettings->school_address }}</p>
                            </div>
                        </div>
                        <div class="flex items-start">
        ...
        <div class="bg-gradient-to-r from-gray-800 to-gray-900 text-white rounded-xl p-8 mt-8">
            ...
            <div class="grid md:grid-cols-3 gap-6 text-center">
                <div>
                    ...
                    <p class="text-sm opacity-90">{{ $mapSettings->school_address }}</p>
                </div>
...
if (document.getElementById('schoolMap')) {
    var schoolMap = L.map('schoolMap').setView([{{ $mapSettings->latitude }}, {{ $mapSettings->longitude }}], {{ $mapSettings->zoom_level }});
    ...
    // Add school marker
    L.marker([{{ $mapSettings->latitude }}, {{ $mapSettings->longitude }}]).addTo(schoolMap)
        .bindPopup('<div class="p-2"><h5 class="font-bold text-blue-600">{{ $mapSettings->school_name }}</h5><p class="text-sm">{{ $mapSettings->school_address }}</p></div>')
        .openPopup();
}
</script>
@endsection