...
@section('title', 'Peta Sebaran - Admin Panitia')
        ...
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-slate-600 mt-1">{{ $mapSettings->school_name }}</p>
                </div>
            ...
            <div class="space-y-4 lg:space-y-6 order-1 xl:order-2">
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6 border border-slate-100">
                    ...
                    <div class="space-y-3">
                        ...
                        @endphp
                        @foreach(array_slice($wilayah_stats, 0, 10, true) as $wilayah => $total)
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-slate-600">{{ $wilayah }}</span>
...
<script>
// Initialize map
var map = L.map('map').setView([{{ $mapSettings->latitude }}, {{ $mapSettings->longitude }}], {{ $mapSettings->zoom_level }});
...
L.marker([{{ $mapSettings->latitude }}, {{ $mapSettings->longitude }}]).addTo(map)
    .bindPopup('<div class="p-2"><h5 class="font-bold text-purple-600">{{ $mapSettings->school_name }}</h5><p class="text-sm">{{ $mapSettings->school_address }}</p></div>');
