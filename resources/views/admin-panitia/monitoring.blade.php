...
@section('title', 'Monitoring Berkas - Admin Panitia')
        ...
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-3">
                    <form method="POST" action="{{ route('admin-panitia.send-bulk-email') }}" class="inline" onsubmit="return confirm('Kirim email ke semua pendaftar yang disetujui?')">
                        @csrf
                    ...
                    <a href="{{ route('admin-panitia.export') }}" class="bg-gradient-to-r from-green-600 to-green-700 text-white px-6 py-3 rounded-xl hover:from-green-700 hover:to-green-800 transition-all duration-200 shadow-lg">
            ...
            <form method="GET" class="space-y-4 lg:space-y-0 lg:flex lg:flex-wrap lg:gap-4">
                ...
                <div class="flex-1 lg:flex-none lg:min-w-48">
                    ...
                    <select name="status" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-purple-500 text-sm lg:text-base">
                        ...
                        <option value="revision" {{ request('status') == 'revision' ? 'selected' : '' }}>Perbaikan</option>
                    </select>
                </div>
                    ...
                    <tbody class="bg-white divide-y divide-slate-200">
                        ...
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">
                                {{ $p->registration_number ?? $p->no_pendaftaran ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                                {{ $p->name ?? $p->nama_lengkap ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                                {{ $p->major ?? $p->jurusan_pilihan ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="text-xs font-medium text-slate-600">{{ $validDocs }}/{{ $totalDocs }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                @if($p->verification_status == 'approved')
                                    <div class="flex space-x-1">
                                        <form method="POST" action="{{ route('admin-panitia.accept-student', $p->id) }}" class="inline">
                                            @csrf
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <form method="POST" action="{{ route('admin-panitia.send-notification', $p->id) }}" class="inline">
                                                        @csrf
